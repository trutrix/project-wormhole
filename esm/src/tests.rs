...
use crate::records::all::{Armor, ArmorAddon, ArtObject, AttractionRule, BodyPartData, Book, Class, Color, NonPlayerCharacter, Projectile, StaticCollection, VisualEffect};
...
fn test1() {
    ...
    for (id, rr) in esm.records {
        ...
        if rr.header.iden.0 != *b"SCOL" {
            continue;
        } else {
            let set = field_ids.entry(rr.header.iden).or_insert(HashSet::new());
            let tr = StaticCollection::try_from(rr).unwrap();

            println!("{:#?}", tr);

            for f in tr.fields {
                match f {
                    crate::records::all::StaticCollectionField::Unknown(four_cc) => {
                        set.insert(four_cc);
                    }
